...
class Command(BaseCommand):
    help = "Provision a new tenant: schema + migrations + seed data + CEO user."
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument("--notify", action="store_true", default=False, help="Send password reset link to the company contact email after provisioning.")
    ...
    def handle(self, *args, **options):
        ...
        # Setup logging to capture output to a file
        log_file = self._setup_logging(schema)
        ...
        try:
            ...
            error_msg = f"Tenant migrations failed for schema '{schema}': {exc}"
        ...
        self._summary(name, schema, domain, email, plan)
        ...
        self.stdout.write(self.style.SUCCESS(f"\n✅ Onboarding complete! Logs: {log_file}\n"))

    # ── helpers ─────────────────────────────────────────────────────────

    def _setup_logging(self, schema):
        """Setup file-based logging for this onboarding job."""
        from django.conf import settings
    ...
    def _create_ceo(self, company_name, email, password=None):
        """Create CEO user with no usable password; expects password reset via email link."""
        ...
        try:
            ...
            pass
        try:
            user.first_name = company_name.split()[0] if company_name else ''
        except Exception:
            pass
        try:
    ...
    def _summary(self, name, schema, domain, email, plan):
        self.stdout.write("")
        self.stdout.write(self.style.SUCCESS("╔══════════════════════════════════════════╗"))
